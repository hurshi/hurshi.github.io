...
1. 需要用到的几个证书
   ...
   | identity_crl *(optional)* | 过期的证书列表   |
   | private_key         | DomainParticipant 证书私钥，对应上述*identity_certificate* 证书；需要配置到终端中； |
   | password *(optional)*     | 私钥密码，可以将上述 *private_key* 加密，这里传入密码；      |
   
   建议：
   
   1. 在打包终端可执行文件的时候，可以将 identity_ca 打包进去，用来在**发现期间**验证其他 DomainParticipant 证书合法性；
   2. identity_certificate 证书可以在终端第一次运行的时候自动生成，本地保存好私钥；然后将生成的证书上传服务器请求签名，服务器使用根证书签名后返回；后续 DomainParticipant 就可以使用这个证书进行发现服务的身份鉴定；
...
1. 证书初始化
   ...
   sequenceDiagram
     ...
     C->>C: 第一次运行，生成 “私钥A” 及 “CA证书A”（identity_certificate）
   	Note over C: 将“私钥A”储存于安全区域
...
1. 身份鉴定
   ...
   1. 在 DDS Application 初始化，调用 create_participant 方法创建 DomainParticipant 的时候，会进入下面的自检流程，自检完成会生成 identityToken 等，然后发给发现服务：

      ![](/img/posts/dds/secure_discovery_init.png)
   
      1. `validate_local_identity()` `(#70)`:验证本地 DomainParticipant，返回当前 participant 的句柄`IdentityHandle`，以及在 DDS 网络中唯一的身份标识`GUID_t`,
   
   1. 然后进入互相握手环节：
   
      ![](/img/posts/dds/secure_discovery.png)
   
      1. `validate_remote_identity()` `(#71)`：验证远程 DomainParticipant，返回：
         1. `VALIDATION_PENDING_HANDSHAKE_REQUEST`：调用`begin_handshake_request`以继续验证；
         2. `VALIDATION_PENDING_HANDSHAKE_MESSAGE`：等待远程调用，然后调用`begin_handshake_reply `以继续握手；
         3. `VALIDATION_OK` / `VALIDATION_FAILED`：成功/失败；
      ...
      3. `get_shared_secret()`：获得协商后的对称加密密钥；

...
1. 证书初始化
   ...
   sequenceDiagram
     ...
     C->>C: 第一次运行，生成 “私钥A” 及 “证书A”（identity_certificate）
   	Note over C: 将“私钥A”储存于安全区域
     C->>S: 将“证书A”上传至证书服务器
     S->>S: 使用根证书私钥签名“证书A”，得到“证书B”
     S->>C: 返回“证书B”
     C->>C: 保存“证书B”（没有安全要求）
...
1. 身份鉴定
   ...
   > 该步骤发生在上述“发现步骤”第❻步：向多播组指定端口发送信息（包含IP地址，单播端口等）

   <div class="mermaid">
   sequenceDiagram
     autonumber
     participant C1 as 终端 A
     participant C2 as 终端 B
     Note over C1: 准备发数据到多播组，数据内容为 Content
     C1->>C1: 使用“私钥A”加密

...
sequenceDiagram
	participant phoneA
	participant phoneB
	phoneA->>phoneA: DomainParticipant发广播
	phoneB->>phoneB: DomainParticipant发广播
	phoneA->phoneB: 当属于同一个 domainId,则发现成功
	phoneA->phoneB: 交换 DataWrites 与 DataReader 信息，包括 Topic
	phoneA->phoneB: 可以互相通信

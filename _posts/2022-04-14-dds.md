...
3. 按照 RTPS 标准，每个 Participant 需要监听 2个端口；当然，端口号可以并存预设，以及计算规则的修改；
   ...
   这里设计巧妙的一点，不同 domainID 使用的端口号是不同的，所以不同 domain 之间数据是隔离的（不会互相发现）；而且相同 domain 不同终端之间计算出来的端口号是一致的，所以可以直接找到对方而不用过滤；

<div class="mermaid">
sequenceDiagram
	participant A as PhoneA
	participant G as 多播组
	participant B as PhoneB
